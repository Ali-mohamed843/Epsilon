...
import { getTiktokPages } from '@/Api/api';
          ...
          <View className="flex-1">
            ...
            <View className="flex-row gap-3 mt-1">
              {page.followers_count != null && (
                <Text className="text-xs" style={{ color: isPressed ? '#6e226e90' : '#94A3B8' }}>
                  👥 {formatCount(page.followers_count)}
                </Text>
              )}
              {page.likes_count != null && (
                <Text className="text-xs" style={{ color: isPressed ? '#6e226e90' : '#94A3B8' }}>
                  ❤️ {formatCount(page.likes_count)}
                </Text>
              )}
              {page.videos_count != null && (
                <Text className="text-xs" style={{ color: isPressed ? '#6e226e90' : '#94A3B8' }}>
                  🎬 {page.videos_count} videos
                </Text>
              )}
            </View>
          </View>
...
const TiktokPagesScreen = () => {
  ...
  useEffect(() => {
    const fetchPages = async () => {
      ...
      setError('');
      try {
        const result = await getTiktokPages();
        if (result.success) {
        ...
        <View className={`flex-1 bg-white ${isLargeScreen ? 'max-w-lg w-full shadow-xl' : 'w-full'}`}>
          ...
          >
            <View className="flex-row items-center mb-6">
              ...
              <Text className="text-2xl font-semibold text-slate-800">TikTok Pages</Text>
            </View>
            ...
            {!loading && !error && pages.length === 0 && (
              <View className="items-center justify-center py-20">
                <Text className="text-slate-400 text-base">No TikTok pages found.</Text>
              </View>
            )}
...
export default TiktokPagesScreen;